...
/** Collect the font bytes a bundle export needs to embed. */
export async function collectBundleFonts(template: Template) {
	const out: Array<FontRef & { data?: string; format?: string }> = [];
	for (const font of template.fonts) {
		if (font.source !== 'local' || !font.ref) {
			out.push({ ...font });
			continue;
		}
		const stored = await idbGet<StoredFont>(STORE_FONTS, font.ref);
		out.push(
			stored?.bytes
				? { ...font, data: bytesToBase64(stored.bytes), format: stored.format }
				: { ...font }
		);
	}
	return out;
...
/** Install any fonts embedded in an imported bundle, before the template renders. */
export async function installBundleFonts(raw: unknown): Promise<void> {
	const fonts = (raw as any)?.fonts;
	if (!Array.isArray(fonts)) return;
	for (const font of fonts) {
		if (!font?.data || !font?.family) continue;
		const ref = String(font.ref ?? `font:${String(font.family).toLowerCase().replace(/[^a-z0-9]+/g, '-')}`);
		try {
			await installFontBytes(ref, String(font.family), base64ToBytes(String(font.data)), String(font.format ?? 'woff2'));
		} catch {
			// Leave it to the missing-font prompt.
		}
	}
}
