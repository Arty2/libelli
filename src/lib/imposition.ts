import type { Orientation, PrintSettings, SheetOrder } from './types';
...
export function resolveImposition(
	...
	for (const sheet of sheetCandidates(print)) {
		...
		// A fold decides its own arrangement: the eight pages of a mini zine are
		// two rows of four and nothing else, however well another grid would fit.
		// Which way round the paper goes is still the fit's to answer, and `auto`
		// will usually answer it with the one the fold wanted anyway.
